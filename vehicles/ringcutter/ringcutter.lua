...
function init()
  ...
  --LEFLS STUFF
   ...
   cruiserProjectile = config.getParameter("cruiserProjectile")
   cruiserProjectileConfig = config.getParameter("cruiserProjectileConfig")
   cruiserFireCycle = config.getParameter("cruiserFireCycle")
...
function controls()
 ...
 if (vehicle.controlHeld("drivingSeat","PrimaryFire")) then
   if self.fireTimer <= 0 then
    world.spawnProjectile(cruiserProjectile, vec2.add(mcontroller.position(), animator.partPoint("frontGun", "firePoint")), entity.id(), {math.cos(aimAngle), math.sin(aimAngle)}, false, cruiserProjectileConfig)
    self.fireTimer = self.fireTimer + cruiserFireCycle
    ...
    if oldFireTimer > cruiserFireCycle / 2 and self.fireTimer <= cruiserFireCycle / 2 then
      world.spawnProjectile(cruiserProjectile, vec2.add(mcontroller.position(), animator.partPoint("backGun", "firePoint")), entity.id(), {math.cos(aimAngle), math.sin(aimAngle)}, false, cruiserProjectileConfig)
